...
function TopicDetail({ topic, t, onBack, isAdmin, onEdit, onDelete }) {
  ...
  const caricaCommenti = async () => {
    const { data } = await supabase.from("commenti").select("*").eq("topic_id", topic.id).order("created_at", { ascending: false });
    setComments(data || []);
  };
  ...
  const inviaCommento = async () => {
    ...
    setLoading(true);
    await supabase.from("commenti").insert([{ topic_id: topic.id, nome: nome.trim(), paese: paese.trim(), testo: testo.trim() }]);
    setTesto("");
  ...
  return (
    <div style={{ maxWidth: 860, margin: "0 auto" }}>
      ...
      <div style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 20, padding: 36, marginBottom: 24 }}>
        ...
        {topic.external_link && (
          <a href={topic.external_link} target="_blank" rel="noopener noreferrer"
            style={{ display: "inline-flex", alignItems: "center", gap: 8, ...S.btn, ...S.btnDark, textDecoration: "none", marginBottom: 8 }}>
      ...
      {/* Comments list */}
      <h3 style={{ fontFamily: "'Cormorant Garamond', serif", fontSize: 22, color: "#f0ebe3", margin: "0 0 16px" }}>
      ...
      {comments.length === 0 ? (
        ...
        comments.map(c => (
          <div key={c.id} style={{ background: "#111", border: "1px solid #1e1e1e", borderRadius: 14, padding: 22, marginBottom: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                {isAdmin && (
                  <button onClick={() => eliminaCommento(c.id)} style={{ ...S.btn, ...S.btnRed, padding: "4px 10px", fontSize: 11 }}>✕</button>
                )}
              </div>
            </div>
            <p style={{ color: "#aaa", fontSize: 14, lineHeight: 1.7, margin: 0, whiteSpace: "pre-wrap" }}>{c.testo}</p>
          </div>
        ))
      )}
    </div>
...
export default function App() {
  ...
  const caricaTopics = async () => {
    ...
    const { data: commentiData } = await supabase.from("commenti").select("topic_id");
    const withCount = (topicsData || []).map(tp => ({
      ...tp,
      comment_count: (commentiData || []).filter(c => c.topic_id === tp.id).length,
    }));
    setTopics(withCount);
    setLoading(false);
  };
          ...
          <>
            ...
            {loading ? (
              ...
              <div style={{ textAlign: "center", padding: "60px 0", color: "#444" }}>
                ...
                <div style={{ fontSize: 16 }}>{t.noTopics}</div>
              </div>
            ) : (
              filtered.map(tp => (
